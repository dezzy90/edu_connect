...
class DeviceController extends Controller
{
    ...
    public function syncStudents(BiometricDevice $device)
    {
        ...
        try {
            // Increase execution time for bulk sync operations
            set_time_limit(600); // 5 minutes for bulk operations
            
            // Get all active students from the school with biometric IDs
            ...
            if ($students->isEmpty()) {
                return back()->withErrors(['error' => 'No active students with biometric IDs found in this school.']);
            }

            // Use PersonnelManagementService to sync students
            ...
            foreach ($students as $student) {
                ...
                usleep(200000); // 200ms delay between students
            }

            // Log the sync operation
            Log::info("Bulk student sync to device", [
            ...
            if ($failCount === 0) {
                $message = "✅ Successfully synced all {$successCount} students to device '{$device->name}'!";
                return back()->with('success', $message);
            } elseif ($successCount > 0) {
                $message = "⚠️ Partial sync completed: {$successCount} successful, {$failCount} failed out of {$students->count()} students.";
                return back()->with('warning', $message);
            } else {
                $errorMessage = "❌ Failed to sync any students to device. ";
                if (!empty($syncResults) && isset($syncResults[0]['error'])) {
                    $errorMessage .= "Error: " . $syncResults[0]['error'];
                }
